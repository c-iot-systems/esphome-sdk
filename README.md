...
esphome-sdk/
  ...
  .github/workflows/            # validate (PR), release-gate (main), release-please, release (tag)
```
...
- **`release-gate.yml`** — every push to `main` that touches shipped YAML. Runs the real
  `esphome compile` over `tests/validate/`. This is the gate that can still **stop** a bad release:
  it runs while the version is only a proposal, before any tag exists.
- **`release-please.yml`** — every push to `main`. Maintains the release PR; creates the tag and the
