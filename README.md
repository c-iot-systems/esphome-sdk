...
- **`release-gate.yml`** — every push to `main`. Job `compile` runs the real `esphome compile` over
  `tests/validate/`; job `release` (release-please, which maintains the release PR and creates the
  tag) **needs** it. Merging the release PR is itself a push to `main`, so the commit that gets
