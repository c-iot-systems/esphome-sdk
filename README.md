...
- **`release-gate.yml`** — every push to `main`. Job `compile` runs the real `esphome compile` over
  `tests/validate/` and job `contract` re-runs the contract gate against the last released tag; job
  `release` (release-please, which maintains the release PR and creates the tag) **needs** both. Merging the release PR is itself a push to `main`, so the commit that gets
